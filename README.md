...
> **基于 [dztabel-happy/codex-agent](https://github.com/dztabel-happy/codex-agent) 项目优化**  
> 原项目提供了 Codex CLI 的基础操作框架，此增强版增加了**任务状态管理**、**质量判断标准**和**Cron 定时任务协作**能力。
...
完整的状态机定义，支持 Cron 定时唤醒场景：

```
codex_running → review_pending → committed / blocked / waiting_user_decision
```

### 2. 质量判断标准

明确区分 Agent 可以自主处理的问题和需要用户拍板的场景：

- ✅ **自主修复**：代码风格、测试补充、小 bug、文档更新
- ⚠️ **用户拍板**：架构改动、技术栈变更、API 变更、需求偏差

### 3. Cron 协作与去重

避免 Cron 每 10 分钟唤醒时重复通知用户，通过 `wakeKey = <taskId>:<status>:<updatedAt>` 实现去重。
